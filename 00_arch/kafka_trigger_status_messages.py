...
def validate_instance_and_get_max_runid(messages_list, mandator_filter, producer_filter, business_date):
    ...
    if not duplicate_instances.empty:
        dup_indices = duplicate_instances.index.tolist()
        dsf_logger.log_msg(
                ...
                for tp, messages in msg_dict.items():
                    for msg in messages:
                        ...
                        if (cnt == COMMIT_CNT or omsg_off >= snapshot_end_offset):
                            ...
                            consumer.commit(
                                {partition: OffsetAndMetadata(omsg_off, omsg_ts)}
                            )
                            ...
                            for msg in messages:
                                ...
                                try:
                                    ...
                                    if(cnt == COMMIT_CNT or omsg_off >= snapshot_end_offset):
                                        ...
                                        f.flush()
                                        os.fsync(f.fileno())
                                        consumer.commit({partition: OffsetAndMetadata(omsg_off, omsg_ts)})
                    ...
                    if len(matching) > 0 and 'status.reconciliationGroupId' in df_analysis.columns:
                        ...
                        if 'status.instanceIndex' in matching.columns and 'status.totalInstances' in matching.columns and not valid_run_ids.empty:
                            ...
                            if missing:
                                ...
                                error_msg_parts.append(f" - All instances present but validation still failed (check for duplicates)")
            else:
